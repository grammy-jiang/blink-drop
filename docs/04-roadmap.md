...
> **⚠️ Superseded (2026-07-07).** This roadmap predates the PWA pivot. What actually shipped: **M0 proven**, then **MVP-1 = the PWA receiver, released as v0.1.0** — the M0 browser receiver was **promoted to the product**, not discarded. Development then continued to **v0.11.1** — encryption (v0.3), Argon2id (v0.4), sender polish/drag-drop (v0.5), resume (v0.6), security DoS bounds (v0.6.2), **multi-file transfer (v0.7)** with a `.zip` delivery fallback (v0.7.1), **sender simplification (v0.8)**, **v0.10 security hardening** (no-egress `connect-src 'none'` CSP on **both** pages, CBOR-depth/KDF/decompression/UR-seq DoS bounds, Cloudflare response headers, hardened CI + CodeQL + OpenSSF Scorecard + weekly Stryker mutation, and — in **v0.10.1** — **Argon2id promoted to the DEFAULT KDF** with PBKDF2 as the opt-out), and the **v0.11 warm terracotta/serif visual restyle** — see [`../CHANGELOG.md`](../CHANGELOG.md) and plans [`06`](06-implementation-plan-v0.2.md)–[`21`](21-sender-playing-focus.md). **Everything except the native iOS receiver has shipped, and the PWA receiver *is* the product**; the single remaining pre-**v1.0** item is the **native iOS receiver** (still roadmap — needs a Mac) plus the **on-hardware real-device test (T5)** that gates the v1.0 cut. The **native-iOS milestones (M1–M4) below are deferred** (no Mac). Pivot delta: [`blink-drop-architecture-update.md`](blink-drop-architecture-update.md).
