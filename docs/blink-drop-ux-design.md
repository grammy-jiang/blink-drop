...
6. **Complete → Result card** (confirmed decision): a card with **name, size, type, and a verified ✓ badge**, and three actions — **Share** (iOS share sheet), **Save to Files**, **Discard**. The file is exposed only here, only after the SHA-256 gate. *(v0.7 multi-file variant — see banner: an **N-file list** with **Share .zip** / **Save .zip** / Discard, every file independently verified.)*
