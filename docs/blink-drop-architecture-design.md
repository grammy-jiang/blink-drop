...
>
> **⚠️ Also amended by update-6 (2026-07-08): v0.10 security hardening.** The receiver's egress is now **`connect-src 'none'` on BOTH pages** (no network at all), **superseding** update-1's `app_fetch_only` / SG-4′ `connect-src 'self'`; a CI invariant gate enforces it. **Argon2id is now the DEFAULT KDF (v0.10.1)**, PBKDF2 the opt-out — treat every "PBKDF2 is the default" line (update-3, ADR-0011) as **superseded**. Added: CBOR nesting-depth bound (32), KDF-parameter (KDF-bomb) bounds, decompression-bomb cap, UR seq-count DoS guard, Cloudflare response headers (frame-ancestors / X-Frame-Options / Referrer-Policy / Permissions-Policy), and hardened CI (SHA-pinned actions + harden-runner + least-privilege + CodeQL + OpenSSF Scorecard + weekly Stryker mutation + branch protection). Testing expanded to cross-browser Playwright E2E (chromium/firefox/webkit) + visual-contract + pixel + coverage gates — read §19 as **superseded** at the web layer. Details: the update note's **Update-6 addendum**, [`../CHANGELOG.md`](../CHANGELOG.md), and [`19-security-hardening-plan.md`](19-security-hardening-plan.md).
>
> **⚠️ Also amended by update-7 (2026-07-09): v0.11 visual restyle.** Both pages were reskinned with a warm "Anthropic-inspired" palette — ivory paper `#faf9f5`, warm near-black ink `#141413`, terracotta accent `#b0512e` (light) / clay `#d97757` (dark) — a **Georgia serif** brand wordmark, WCAG-AA contrast, and dark mode via a `prefers-color-scheme` CSS-custom-property token swap; app icons + favicon warmed to match. This replaced the earlier cool blue/gray look. **No behaviour, protocol, or state-model change** — read §23 Experience Architecture's visual direction as this warm palette. Details: the update note's **Update-7 addendum** and [`../CHANGELOG.md`](../CHANGELOG.md).
