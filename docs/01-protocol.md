...
- **Malformed input is a failure, not a crash.** Invalid Bytewords, non-conforming dCBOR, header missing a required key, or a media type / name that is not well-formed → drop the part or fail the transfer cleanly. A malformed UR part is **dropped** (`Assembler.receiveQr` returns `false`, never throws — a garbled or hostile QR frame can't crash the scan loop). Strict at the boundary (blueprint's "crash early / distrust input").
- **Untrusted filename (receiver delivery guard).** The reconstructed *bytes* are SHA-256-verified, but the sender-controlled `name` is **not** trusted: before it is used as a download name, a share-sheet filename, or a `.zip` entry key, the receiver reduces it to a **basename** (drops path components — no `../` traversal / zip-slip), strips control characters, trims leading/trailing dots, and caps its length. See `12`/`16-security-review-response.md`.
