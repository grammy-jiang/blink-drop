...
| | |
|---|---|
| **Status** | Draft v0.7 — opt-in passphrase encryption + resume + multi-file shipped |
| **Date** | 2026-07-07 |
...
**U2 confidentiality scope.** A *plaintext* transfer provides no payload confidentiality — the QR animation is readable by anything with line of sight or a screen recorder. **As of v0.3, an opt-in passphrase encrypts the content and metadata** (DEC-1 reversed, §13); its residual leaks (transfer size and that a transfer happened, plus the passphrase-strength ceiling) are stated honestly in the UI, not implied away. For a plaintext transfer, U2 remains limited to material that is non-secret, already encrypted at rest, or whose optical exposure the user explicitly accepts — moving cleartext secrets unencrypted is the user's explicit risk.
...
| 4 | **Optical eavesdropping** — anything with a camera and sightline (or a screen recorder) captures a *plaintext* payload; U2 (credentials) is the exposed case | v0.1/v0.2 shipped with **no confidentiality** (DEC-1); **v0.3 adds opt-in passphrase encryption** that seals content + metadata (DEC-1 reversed, §13). Residual leaks (transfer size/occurrence) and the passphrase-strength ceiling are documented; a *plaintext* transfer keeps the original exposure. See also Risk 7. |
...
- **DEC-1 — ship v1 without payload confidentiality. DECIDED 2026-07-07: yes (v0.1/v0.2). REVERSED in v0.3 (2026-07-07):** opt-in passphrase encryption now ships as an additive layer that does not change the transport framing (`docs/07-implementation-plan-v0.3-encryption.md`; architecture update-2). The original rationale (fastest path to a working end-to-end system) held for v0.1/v0.2; the encrypted path seals content + metadata, with residual size/occurrence leaks documented. Plaintext remains the default.
...
- **Trust, control, transparency:**
  ...
  - Plaintext transfers claim no confidentiality (DEC-1). **v0.3 adds opt-in passphrase encryption**; when used, the UI may indicate encryption but must state the honest limits (size and that a transfer happened stay visible) — it must never overclaim privacy.
